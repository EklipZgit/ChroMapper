...
public class NJSEventGridContainer : BeatmapObjectContainerCollection<BaseNJSEvent>
{
    ...
    internal override void SubscribeToCallbacks()
    {
        ...
        AudioTimeSyncController.OnTimeChanged += UpdateHJDLine;
    ...
    protected override void HandleObjectSpawned(BaseObject _, bool __ = false) => UpdateHJDLine();

    protected override void HandleObjectDelete(BaseObject _, bool __ = false) => UpdateHJDLine();
    ...
    public void UpdateHJDLine()
    {
        ...
        var baseHJD = SpawnParameterHelper.CalculateHalfJumpDuration(
            baseNJS,
        ...
        {
            var previousNonExtendNJSEvent =
                MapObjects.FindLast(x => x.UsePrevious == 0 && x.JsonTime <= previousNJSEvent.JsonTime);
        ...
        var easingId = nextNJSEvent?.Easing switch
        {
            >= 4 and <= 18 => 0,
            _ => nextNJSEvent?.Easing ?? 0
        };
        var easing = Easing.FromID(easingId);
        var lerpPoint = easing(easingPoint);
