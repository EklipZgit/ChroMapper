...
using System.Linq;
using Beatmap.Base;
// V3ColorConversionToV4DropsGeneratedEasingFromOutputOnly reuses the V3 extension's reserved key/ID contract.
using Beatmap.V3;
using SimpleJSON;
using UnityEngine;
            ...
            IList<V4CommonData.LightColorEvent> lightColorEventsCommonData)
        {
            ...
            foreach (var boxEvent in group.Boxes)
            {
                ...
                foreach (var evt in boxEvent.Events)
                {
                    ...
                    // V3ColorConversionToV4DropsGeneratedEasingFromOutputOnly keeps native V4 e authoritative:
                    // strip the reserved V3 extension only from an owned output snapshot, never source/model data.
                    evt.CustomData = evt.SaveCustom();
                    var customData = evt.CustomData;
                    if (customData.HasKey(V3LightColorBase.CustomKeyEasing))
                    {
                        // V4ColorSavePreservesNonGeneratedEasingMetadata leaves strings, fractions and unknown
                        // numeric IDs alone because they were not generated by the V3 color serializer.
                        var easing = customData[V3LightColorBase.CustomKeyEasing];
                        if (easing.IsNumber && easing.AsDouble == easing.AsInt
                            && V3LightColorBase.RequiresCustomEasing(easing.AsInt))
                        {
                            customData = customData.Clone();
                            customData.Remove(V3LightColorBase.CustomKeyEasing);
                        }
                    }

                    // V4NativeColorEasingWinsOverStaleImportedV3Metadata omits an easing-only object after
                    // cleanup while retaining colors and all unrelated extension fields on the output node.
                    if (customData.Children.Any())
                        eventNode["customData"] = customData;
