...
public class BookmarkManager : MonoBehaviour, CMInput.IBookmarksActions
{
    ...
    public event Action<BaseObject> OnBookmarkAdded;
    public event Action<BaseObject> OnBookmarkDeleted;
    ...
    private IEnumerator Start()
    {
        ...
        createBookmarkDialogBox = PersistentUI
            .Instance
            .CreateNewDialogBox()
    ...
    private void RefreshBookmarksFromOnLoadedDifficulty()
    {
    ...
    public void NotifyUpdate() => OnBookmarksUpdated?.Invoke();

    private List<BookmarkContainer> InstantiateBookmarkContainers()
    {
        return BeatSaberSongContainer
            .Instance.Map.Bookmarks.Select(bookmark =>
            {
    ...
    internal void AddBookmark(BaseBookmark bookmark, bool triggerEvent = true)
    {
        ...
        if (triggerEvent) OnBookmarkAdded?.Invoke(bookmark);
    }
    ...
    internal void DeleteBookmark(BookmarkContainer container, bool triggerEvent = true)
    {
        ...
        if (triggerEvent) OnBookmarkDeleted?.Invoke(container.Data);
    }
