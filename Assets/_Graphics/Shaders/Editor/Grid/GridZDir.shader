...
Shader "ChroMapper/Editor/Grid/Grid ZDir"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            v2f vert(appdata input)
            {
                ...
                float newX = (output.worldPos.x - offset.x) * cos(rotationInRadians) - (output.worldPos.z - offset.z) *
                    sin(rotationInRadians);
                float newZ = (output.worldPos.z - offset.z) * cos(rotationInRadians) + (output.worldPos.x - offset.x) *
                    sin(rotationInRadians);
