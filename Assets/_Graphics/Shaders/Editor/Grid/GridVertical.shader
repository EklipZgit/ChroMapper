Shader "ChroMapper/Editor/Grid/Grid Vertical"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 frag(v2f i) : SV_Target
            {
                ...
                if (_Directions.x != 0)
                {
                    if ((abs(xPos) % gridSpacing) / gridSpacing <= gridThickness / 2 || (abs(xPos) % gridSpacing) /
                        gridSpacing >= 1 - (gridThickness / 2))
                    {
                ...
                if (_Directions.y != 0)
                {
                    if ((abs(yPos) % gridSpacing) / gridSpacing <= gridThickness / 2 || (abs(yPos) % gridSpacing) /
                        gridSpacing >= 1 - (gridThickness / 2))
                    {
                        baseColour.a = gridAlpha;
                    }
