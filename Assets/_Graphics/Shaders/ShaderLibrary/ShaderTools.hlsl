...
inline half4 RGBtoHSL(half4 rgb)
{
    half4 hsl = half4(0.0, 0.0, 0.0, rgb.w);

    half vMin = min(min(rgb.x, rgb.y), rgb.z);
    half vMax = max(max(rgb.x, rgb.y), rgb.z);
    half vDelta = vMax - vMin;
...
inline half hueToRGB(float v1, float v2, float vH)
{
    if (vH < 0.0) vH += 1.0;
    if (vH > 1.0) vH -= 1.0;
...
inline half4 HSLtoRGB(half4 hsl)
{
    half4 rgb = half4(0.0, 0.0, 0.0, hsl.w);
